...
INITIAL_BOARD = [WHITE | ROOK, WHITE | KNIGHT, WHITE | BISHOP, WHITE | QUEEN, WHITE | KING, WHITE | BISHOP,
...
def alpha_beta(game, color, depth, alpha=-float('inf'), beta=float('inf')):
    ...
    if color == WHITE:
        for move in legal_moves(game, color):
            if verbose:
                print('\t' * depth + str(depth) + '. evaluating ' + PIECE_CODES[
...
def get_AI_move(game, depth=2):
    ...
    end_time = time()
    if verbose:
        print('Found move ' + PIECE_CODES[get_piece(game.board, move[0])] + ' from ' + str(
